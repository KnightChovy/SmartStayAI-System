  ...
  return (
    <SafeAreaView className="flex-1 bg-navy" edges={['top']}>
      <View className="flex-row items-center gap-3 px-4 pt-2 pb-3 bg-white border-b border-gray-100">
        ...
        <Heading size="lg" className="text-navy">My Offers</Heading>
      </View>

      <ScrollView className="flex-1 bg-gray-100" contentContainerStyle={{ padding: 16, gap: 12 }} showsVerticalScrollIndicator={false}>
        <Text size="sm" className="text-gray-500 px-1 mb-1">
          Apply a code at checkout to redeem your discount.
        </Text>
        {OFFERS.map((offer) => (
          <View key={offer.id} className="bg-white rounded-2xl p-4">
            <View className="flex-row items-center gap-2">
              <Ionicons name="pricetag-outline" size={18} color={NAVY} />
              <Text bold className="text-navy text-base">{offer.title}</Text>
            </View>
            <Text size="sm" className="text-gray-500 mt-1">{offer.description}</Text>
            <Text size="xs" className="text-gray-400 mt-1.5">Valid until {offer.validUntil}</Text>

            <View className="mt-3 flex-row items-center justify-between rounded-xl border border-dashed border-gray-300 bg-gray-50 px-4 py-2.5">
              <Text bold className="text-navy tracking-wider">{offer.code}</Text>
              <Text size="xs" className="text-gray-400">Enter at checkout</Text>
            </View>
          </View>
        ))}
      </ScrollView>
