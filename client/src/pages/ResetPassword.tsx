    ...
    <div className="relative min-h-screen text-on-surface select-none overflow-hidden h-screen w-screen bg-background antialiased flex items-center justify-center px-margin-mobile">
      ...
      <main className="relative z-10 w-full max-w-md">
        ...
        <div
          ...
          style={{
            transform: passwordFocused || confirmPasswordFocused ? 'scale(1.01)' : 'scale(1)',
        ...
        >
          ...
          {!token ? (
            <div className="bg-error/10 border border-error/20 text-error p-4 rounded-xl text-center font-body-md mb-6">
              ...
              <p className="text-sm mt-1">This token is missing or has expired. Please request a new link.</p>
              <Link to="/forgot-password" className="mt-4 inline-block text-secondary font-bold hover:underline">
            ...
            <div className="bg-green-800/10 border border-green-800/20 text-green-700 p-4 rounded-xl text-center font-body-md mb-6">
              <span className="material-symbols-outlined text-green-800 text-3xl mb-2 block">check_circle</span>
              <p className="font-semibold">Password Reset Successful!</p>
              <p className="text-sm mt-1">Your password has been changed successfully. Redirecting you to login...</p>
            </div>
          ) : (
            <form className="space-y-stack-md" onSubmit={handleFormSubmit(onSubmit)}>
              {resetPasswordError && (
                <div className="bg-error/10 border border-error/20 text-error p-3 rounded-xl text-sm font-semibold">
                  {(resetPasswordError as any)?.response?.data?.message || 'Failed to reset password. Please try again.'}
                </div>
              )}
              ...
              <div>
                ...
                />
                {errors.password && (
                  <p className="text-[10px] text-error font-semibold mt-1.5 px-1">{errors.password.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                {errors.confirmPassword && (
                  <p className="text-[10px] text-error font-semibold mt-1.5 px-1">{errors.confirmPassword.message}</p>
                )}
              </div>
