...
import { WalletPanel } from '@/components/hotel-partner/wallet/WalletPanel';
import {
  EmptyState,
  ErrorState,
...
/**
 * Wallet (`/partner/wallet`) — số dư + sổ giao dịch của MỘT khách sạn.
 * Ví là per-hotel ở backend nên vẫn phải chọn khách sạn trước (`?hotelId=`).
 */
export default function WalletPage() {
    ...
    return (
      <Shell>
        <EmptyState
          ...
          description="Complete hotel verification and get approved before viewing your wallet."
          action={
            <Button
  ...
  if (!currentHotel) {
    return (
      <Shell>
        <Header
          title="Wallet"
          subtitle="Pick a hotel to see its balance and transaction history."
        />
        <HotelDirectory
          hotels={activeHotels}
  ...
  return (
    <Shell>
      ...
      <WalletPanel hotelId={currentHotel.id} />
    </Shell>
  );
}
  ...
  return (
    <div className="mb-6 flex items-center gap-3">
      <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-role-partner-light">
        <Wallet className="h-6 w-6 text-role-partner-primary" />
      </div>
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">
          {title}
