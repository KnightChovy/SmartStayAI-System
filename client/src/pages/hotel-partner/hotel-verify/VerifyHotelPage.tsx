...
export default function VerifyHotelPage() {
  ...
  // The current step is derived directly from the URL — single source of truth,
  // so a reload (or back/forward) restores the same step without extra state.
  const step = parseInt(searchParams.get('step') || '0', 10);

  const [hasSubmitted, setHasSubmitted] = useState(
    !applicationId && step === 0
  );
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-6 max-w-350 mx-auto w-full">
      {hasSubmitted && !applicationId ? (
        ...
        <div className="w-full mx-auto max-w-250 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="w-full mx-auto max-w-150 animate-in fade-in zoom-in-95 duration-500 py-16 text-center">
          ...
          <p className="text-slate-600 mb-8 text-lg">
            ...
            review your documents and respond within 2-3 business days.
          </p>
          <Button
            onClick={() => {
              updateStep(0);
        ...
        <div className="w-full mx-auto max-w-250 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          {step === 1 && <BusinessInfoStep onContinue={() => updateStep(2)} />}
          ...
          {step === 7 && (
            <ReviewSubmitStep
              onBack={() => updateStep(6)}
              onSubmit={() => updateStep(8)}
              onNavigateToStep={updateStep}
            />
          )}

          <div className="text-center mt-8 text-sm text-slate-500 font-medium">
            Need help? Contact{' '}
            <span className="text-role-partner-primary cursor-pointer hover:underline">
              Partner Support
            </span>
          </div>
        </div>
