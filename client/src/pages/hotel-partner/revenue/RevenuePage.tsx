...
import { Banknote, ShieldCheck, ArrowLeft } from 'lucide-react';
...
import { RevenueTab } from '@/components/hotel-partner/revenue/RevenueTab';
...
/**
 * Revenue (`/partner/revenue`) — báo cáo doanh thu + ví cho MỘT khách sạn của partner.
...
export default function RevenuePage() {
    ...
    return (
      <Shell>
        <EmptyState
          ...
          description="Complete hotel verification and get approved before viewing revenue."
          action={
            <Button
  ...
  if (!currentHotel) {
    return (
      <Shell>
        <Header
          title="Revenue"
          subtitle="Select a hotel to view its revenue report and wallet."
        />
        <HotelDirectory hotels={activeHotels} onManage={selectHotel} actionLabel="View revenue" />
      </Shell>
    );
  }
  ...
  return (
    <Shell>
      ...
      <RevenueTab hotelId={currentHotel.id} />
    </Shell>
  );
}
  ...
  return (
    <div className="mb-6 flex items-center gap-3">
      <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-role-partner-light">
        <Banknote className="h-6 w-6 text-role-partner-primary" />
      </div>
      <div>
