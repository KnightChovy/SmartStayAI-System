...
export default function Register() {
  ...
  const { sendOtp, isSendingOtp, sendOtpError } = useAuth();
          ...
          <div className="glass-panel rounded-3xl p-stack-lg shadow-[0_4px_20px_rgba(26,26,26,0.04)] border border-white/20">
            ...
            <form className="space-y-stack-md" onSubmit={handleFormSubmit(onSubmit)}>
              {sendOtpError && (
                <div className="bg-error/10 border border-error/20 text-error p-3 rounded-xl text-sm font-semibold">
                  {(sendOtpError as any)?.response?.data?.message || 'Failed to send OTP code. Please try again.'}
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                {errors.name && <p className="text-error text-xs font-semibold mt-1.5 ml-1">{errors.name.message}</p>}
              </div>
              ...
              <div className="space-y-2">
                ...
                {errors.confirmPassword && (
                  <p className="text-error text-xs font-semibold mt-1.5 ml-1">{errors.confirmPassword.message}</p>
                )}
              </div>
            ...
            <footer className="mt-stack-lg text-center space-y-4">
              <p className="font-body-md text-body-md text-on-surface-variant">
                ...
                <Link className="text-on-surface font-semibold hover:underline transition-all" to="/login">
                  Login
                </Link>
              </p>
