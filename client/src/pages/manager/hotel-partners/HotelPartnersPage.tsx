...
const mockPartners: HotelPartner[] = [
  ...
  },
  {
    ...
    monthlyRevenue: '₫45M',
    occupancyRate: 38,
    cancellationRate: 31.5,
    responseTimeHrs: 18.4,
  },
  {
    id: 'HP-004',
    ownerName: 'Phạm Thị Thu',
  ...
  {
    id: 'HP-005',
    ownerName: 'Hoàng Văn Nam',
    hotelName: 'Gold Coast Hotel',
    location: 'Đà Nẵng',
    joinedAt: '2025-07-14',
    status: 'active',
    totalBookings: 620,
...
const occupancyData = mockPartners
  ...
  .map(p => ({
    name: p.hotelName.split(' ').slice(0, 2).join(' '),
    occupancy: p.occupancyRate,
    cancellation: p.cancellationRate,
  }));
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { label: 'Joined', value: partner.joinedAt, icon: CalendarDays },
              {
                ...
                value: partner.totalBookings.toLocaleString(),
                icon: CalendarDays,
              },
              {
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="bg-slate-50 rounded-xl p-3 text-center">
                  <p className="text-lg font-bold text-role-manager-primary">
                    {partner.occupancyRate}%
                  </p>
                ...
                <div
                  className={cn(
                    ...
                    partner.cancellationRate > 15 ? 'bg-red-50' : 'bg-slate-50'
                  )}
                >
                  <p
                    className={cn(
                      'text-lg font-bold',
                      partner.cancellationRate > 15
                        ? 'text-red-600'
                        : 'text-slate-800'
                    )}
                  >
                ...
                <div
                  className={cn(
                    'rounded-xl p-3 text-center',
                    partner.responseTimeHrs > 4 ? 'bg-amber-50' : 'bg-slate-50'
                  )}
                >
                  <p
                    className={cn(
                      'text-lg font-bold',
                      partner.responseTimeHrs > 4
                        ? 'text-amber-600'
                        : 'text-slate-800'
                    )}
                  >
                    {partner.responseTimeHrs}h
                  </p>
          ...
          <div className="flex items-center justify-between bg-slate-50 rounded-xl p-3">
            ...
            <div
              className={cn(
                ...
                partner.violations > 2
                  ? 'bg-red-100 text-red-700'
                  : 'bg-slate-100 text-slate-600'
              )}
            >
              <AlertTriangle className="w-3 h-3" />
              {partner.violations} violation
              {partner.violations !== 1 ? 's' : ''}
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4">
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          <div className="flex gap-2 flex-wrap">
            {(['all', 'active', 'suspended', 'pending'] as PartnerStatus[]).map(
              s => (
                <button
                  key={s}
                  onClick={() => setFilter(s)}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-slate-700 font-medium">
                        {p.totalBookings.toLocaleString()}
                      </td>
                      <td className="px-5 py-4">
                        {p.rating > 0 ? (
...
function PlatformPerformanceTab({ partners }: { partners: HotelPartner[] }) {
  ...
  const avgOccupancy = (
    active.reduce((s, p) => s + p.occupancyRate, 0) / active.length
  ).toFixed(1);
  const avgCancellation = (
    active.reduce((s, p) => s + p.cancellationRate, 0) / active.length
  ).toFixed(1);
  const avgResponse = (
    active.reduce((s, p) => s + p.responseTimeHrs, 0) / active.length
  ).toFixed(1);
  const avgRating = (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            color: 'text-red-500',
            bg: 'bg-red-50',
            good: Number(avgCancellation) < 10,
          },
          {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          ...
          <p className="text-xs text-slate-400 mb-5">
            Aggregate performance across all dimensions
          </p>
          <ResponsiveContainer width="100%" height={220}>
            ...
            <tbody className="divide-y divide-slate-50">
              {active.map(p => {
                const score = Math.round(
                  p.occupancyRate * 0.4 +
                    (100 - p.cancellationRate * 2) * 0.3 +
                    p.rating * 10 * 0.2 +
                    (10 - Math.min(p.responseTimeHrs, 10)) * 10 * 0.1
                );
                const scoreColor =
                  score >= 80
                    ? 'text-emerald-600'
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <span
                        className={cn(
                          'font-medium',
                          p.cancellationRate > 15
                            ? 'text-red-600'
                            : p.cancellationRate > 8
                              ? 'text-amber-600'
                              : 'text-emerald-600'
                        )}
                      >
...
function BookingActivitiesTab({
  ...
  const [statusFilter, setStatusFilter] = useState<
    'all' | 'confirmed' | 'cancelled' | 'completed' | 'pending'
  >('all');
  ...
  const filtered = bookings.filter(b => {
    ...
    const matchPartner =
      partnerFilter === 'all' || b.partnerId === partnerFilter;
