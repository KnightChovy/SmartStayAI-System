...
export function RoomAvailability({
  ...
  return (
    <section className="bg-white dark:bg-card p-6 md:p-8 rounded-3xl border border-outline-variant/20 shadow-sm" id="availability">
      <h2 className="font-bold text-xl mb-6 text-on-surface">Available Rooms</h2>
      ...
      <div className="bg-surface-container-low p-5 rounded-2xl mb-8 grid grid-cols-1 md:grid-cols-4 gap-4 items-end border border-outline-variant/10">
        <div className="space-y-1.5">
          <label className="text-[10px] text-outline font-bold uppercase px-1 tracking-wider">Check-in Date</label>
          <input
        ...
        <div className="space-y-1.5">
          <label className="text-[10px] text-outline font-bold uppercase px-1 tracking-wider">Check-out Date</label>
          <input
            className="w-full bg-white dark:bg-card border border-outline-variant/35 rounded-xl p-2.5 text-sm font-semibold focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all cursor-pointer"
            type="date"
            value={checkOut}
        ...
        <Button
          className="bg-primary text-white hover:bg-on-surface py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest hover:scale-[1.01] active:scale-95 transition-all h-[44px] cursor-pointer"
          onClick={() => alert(`Searching for reservations from ${formatDisplayDate(checkIn)} to ${formatDisplayDate(checkOut)} for ${guests}...`)}
        >
          Search
        </Button>
      </div>
      ...
      <div className="space-y-4">
        ...
        >
          <div className="flex-1">
            <div className="flex items-center gap-2.5 mb-1.5">
              <h3 className="font-bold text-lg text-on-surface">Superior Double Room</h3>
              {selectedRoomId === 'superior' && (
            ...
            <p className="text-on-surface-variant text-xs font-semibold">{rooms.superior.details}</p>
          </div>
          <div className="flex items-center gap-6 self-end md:self-auto shrink-0 mt-3 md:mt-0">
            <div className="flex flex-col text-right">
              <span className="font-bold text-xl text-primary">${rooms.superior.price}</span>
              <span className="text-[9px] text-outline font-bold uppercase tracking-wider">per night</span>
            </div>
            <Button
              className={`px-5 py-2 rounded-full text-2xs font-bold uppercase tracking-wider border-none cursor-pointer h-8 ${
                selectedRoomId === 'superior' ? 'bg-primary text-white' : 'bg-outline text-white hover:bg-primary'
              }`}
              onClick={(e) => {
                e.stopPropagation();
        ...
        >
          <div className="flex-1">
            <div className="flex items-center gap-2.5 mb-1.5">
              <h3 className="font-bold text-lg text-on-surface">Executive Penthouse Suite</h3>
              {selectedRoomId === 'penthouse' && (
                <span className="bg-secondary-container text-on-secondary-container text-[9px] px-2 py-0.5 rounded-full uppercase font-bold tracking-wider">
                  Selected
                </span>
            ...
            <p className="text-on-surface-variant text-xs font-semibold">{rooms.penthouse.details}</p>
          </div>
          <div className="flex items-center gap-6 self-end md:self-auto shrink-0 mt-3 md:mt-0">
            <div className="flex flex-col text-right">
              <span className="font-bold text-xl text-primary">${rooms.penthouse.price}</span>
              <span className="text-[9px] text-outline font-bold uppercase tracking-wider">per night</span>
            </div>
            <Button
              className={`px-5 py-2 rounded-full text-2xs font-bold uppercase tracking-wider border-none cursor-pointer h-8 ${
                selectedRoomId === 'penthouse' ? 'bg-primary text-white' : 'bg-outline text-white hover:bg-primary'
              }`}
              onClick={(e) => {
                e.stopPropagation();
