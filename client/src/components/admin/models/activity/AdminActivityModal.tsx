...
export function AdminActivityModal({
  ...
  return (
    ...
    >
      ...
      <section className="relative z-10 flex h-[min(82vh,640px)] w-full max-w-2xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl">
        <header className="flex items-start justify-between gap-4 border-b border-outline-variant/40 px-4 py-4 sm:px-6">
          <div>
            ...
            <p className="mt-1 text-xs font-semibold text-muted-foreground">
              Platform audit log · {formatDateLong(currentTime)} |{' '}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 sm:px-6">
          ...
          <div className="space-y-2">
            {filteredLogs.map(log => (
              <div
                className="rounded-xl border border-outline-variant/40 bg-slate-50 p-3"
                key={log.id}
              >
