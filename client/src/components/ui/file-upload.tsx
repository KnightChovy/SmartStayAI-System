    ...
    <div className="space-y-3">
      ...
      <div 
        className={cn(
          ...
          isLarge ? "h-64" : "h-32",
          isDragging ? "border-role-partner-primary bg-role-partner-primary/5" : "border-slate-300"
        )}
      ...
      {files.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
          {files.map((file, idx) => (
            <div key={idx} className="flex items-center gap-3 p-2 bg-white border border-slate-200 rounded-lg shadow-sm relative pr-8 group">
              {isImage(file) ? (
