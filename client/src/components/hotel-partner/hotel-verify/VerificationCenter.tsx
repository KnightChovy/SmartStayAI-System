...
import { Hourglass, CheckCircle2, Search, Loader2, Plus, AlertCircle } from 'lucide-react';
    ...
    <div className="w-full max-w-5xl mx-auto">
      ...
      {latestApp ? (
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-8 border-b border-slate-100 pb-5">
            <div className="flex items-start gap-3">
              <div className={cn("w-10 h-10 rounded-full flex items-center justify-center shrink-0 border", 
                latestApp.overallStatus === StepStatus.REJECTED ? "bg-red-100 border-red-200" : "bg-amber-100 border-amber-200"
              )}>
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <h2 className="text-lg font-bold text-slate-900">
                    {latestApp.overallStatus === StepStatus.REJECTED ? 'Action Required' : 'Review in Progress'}
                  </h2>
                  <span className={cn("text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded border",
                    latestApp.overallStatus === StepStatus.REJECTED ? "bg-red-100 text-red-700 border-red-200" : "bg-amber-100 text-amber-700 border-amber-200"
                  )}>
            ...
            {latestApp.overallStatus === StepStatus.REJECTED ? (
              <Button onClick={() => handleEdit(latestApp.id)} size="sm" className="bg-role-partner-primary hover:bg-role-partner-secondary text-white">
                Review & Fix
              </Button>
            ) : (
          ...
          <div className="pl-1 space-y-6 relative">
            ...
            <div className="flex gap-4 relative z-10">
              ...
              <div>
                ...
                <p className="text-[13px] text-slate-600 mb-1.5">You successfully uploaded required documents.</p>
            ...
            <div className="flex gap-4 relative z-10">
              ...
              <div>
                ...
                {latestApp.steps?.businessInfo?.status === StepStatus.REJECTED && (
                  <div className="bg-red-50 border border-red-100 p-3 rounded-md mt-2">
                    ...
                    <p className="text-xs text-red-600 mt-1">{latestApp.steps.businessInfo.rejectReason || 'Please review this section'}</p>
                    <Button onClick={() => handleEdit(latestApp.id)} variant="link" className="text-red-700 h-auto p-0 text-xs mt-2 font-bold">Fix Now &rarr;</Button>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="border border-slate-200 rounded-xl p-10 bg-slate-50 text-center max-w-3xl border-dashed">
          <h3 className="text-lg font-semibold text-slate-700 mb-2">No Verification Applications</h3>
          <p className="text-sm text-slate-500 mb-6 max-w-md mx-auto">
            You haven't submitted any properties for verification yet. Start the process to get your hotel listed.
          </p>
          <Button onClick={onVerifyNew} className="bg-role-partner-primary hover:bg-role-partner-secondary text-white">
