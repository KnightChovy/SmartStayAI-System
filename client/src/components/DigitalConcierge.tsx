...
export default function DigitalConcierge() {
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    setIsTyping(true)
    setTimeout(() => {
      let aiReply = "I would be delighted to assist you. Smart Stay AI can curate a custom itinerary for you. Would you prefer a beach sanctuary in Phu Quoc, a cultural experience in Hoi An, or a French villa in Da Lat?"

      if (typedText.includes("bali")) {
        aiReply = "Bali is a wonderful choice! We have 1,240 verified luxury properties there, including clifftop villas in Uluwatu and spiritual sanctuaries in Ubud. Let me know if you would like me to lock in members-only rates."
      } else if (typedText.includes("paris")) {
        aiReply = "Paris exudes classic sophistication. We feature 890 curated penthouses and boutique hotels overlooking the Seine. Would you like a view of the Eiffel Tower for your stay?"
      } else if (typedText.includes("vietnam") || typedText.includes("hạ long") || typedText.includes("ha long") || typedText.includes("hội an") || typedText.includes("hoi an") || typedText.includes("đà lạt") || typedText.includes("da lat") || typedText.includes("phú quốc") || typedText.includes("phu quoc")) {
        aiReply = "Vietnam offers some of our finest Quiet Luxury escapes. From emerald cruises in Ha Long Bay, heritage lantern villas in Hoi An, pine retreats in Da Lat, to sunset spa resorts in Phu Quoc. What type of scenery are you dreaming of?"
      } else if (typedText.includes("deal") || typedText.includes("khuyến mãi") || typedText.includes("giá") || typedText.includes("price")) {
        aiReply = "We currently have outstanding weekend deals! For instance, 'The Azure Sanctuary' in the Maldives is -15% off, and the 'Timberline Retreat' in Aspen is -20% off. You can see these under our 'Deals' section."
      } else if (typedText.includes("hello") || typedText.includes("hi") || typedText.includes("xin chào")) {
        aiReply = "Hello! I am your AI-powered Concierge. I can help you find exclusive rates, book boutique hotels, and plan your luxury travel itineraries. Where would you like to explore?"
      }
  ...
  return (
    <div className="fixed bottom-8 right-8 z-[100] group font-be-vietnam">
      ...
      >
        ...
        <div className="bg-on-surface p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/60 text-[9px] uppercase tracking-widest font-be-vietnam">Always Online</p>
            </div>
          </div>
          <button 
          ...
          {messages.map((msg, index) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[9px] text-on-surface-variant/60 mt-1 ${isAI ? "text-left" : "text-right"}`}>
                  {msg.time}
                </span>
              </div>
            )
        ...
        {/* Input Area */}
        <form onSubmit={handleSend} className="p-4 border-t border-outline-variant/20 bg-white">
