      ...
      <div className="flex items-center gap-6 overflow-x-auto hide-scrollbar pb-6">
        {types.map((type) => {
          ...
          return (
            ...
            >
              <div className={`w-full aspect-square rounded-3xl bg-surface-container overflow-hidden ring-2 transition-all duration-300 shadow-sm ${isSelected ? "ring-primary" : "ring-outline-variant/20 group-hover:scale-105"}`}>
                <img 
                  alt={type.name} 
              ...
              <span className={`text-sm font-bold font-be-vietnam ${isSelected ? "text-primary" : "text-on-surface"}`}>
                {type.name}
              </span>
            </button>
          )
