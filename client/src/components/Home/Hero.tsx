...
export default function Hero() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    alert(`Searching for: ${destination || "Anywhere"} | Dates: ${dates} | Guests: ${guests}`)
  }
  ...
  return (
    <section className="relative pt-20 pb-24 px-margin-mobile overflow-hidden bg-gradient-to-b from-surface to-transparent w-full">
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        <form onSubmit={handleSearch} className="bg-white p-2 rounded-3xl md:rounded-full shadow-2xl flex flex-col md:flex-row items-center gap-2 border border-outline-variant/20">
          ...
          <div className="w-full md:flex-1 px-6 py-3 flex flex-col cursor-pointer" onClick={() => {
            const dateInput = prompt("Enter dates (e.g., Jun 12 - Jun 15):", dates === "Add dates" ? "" : dates)
          ...
          <div className="hidden md:block w-px h-8 bg-outline-variant/30"></div>
          <div className="w-full md:flex-1 px-6 py-3 flex flex-col cursor-pointer" onClick={() => {
            const guestInput = prompt("Enter number of guests (e.g., 2 guests, 1 room):", guests === "Add guests" ? "" : guests)
